...
boyle_reg_1 <- lm(above_median ~ boyle_dummy + sentence_date + sentence_year.f, data = model_1_data)
cov <- vcovHC(boyle_reg_1, type = "HC") 
boyle_reg_1_robust.se <- sqrt(diag(cov))
boyle_reg_2 <- lm(counter_F4_pj ~ boyle_dummy + sentence_date + sentence_year.f, data = model_2_data)
cov <- vcovHC(boyle_reg_2, type = "HC") 
boyle_reg_2_robust.se <- sqrt(diag(cov))
boyle_reg_3 <- lm(severe_sentence ~ boyle_dummy + sentence_date + sentence_year.f, data = model_3_data)
cov <- vcovHC(boyle_reg_3, type = "HC") 
boyle_reg_3_robust.se <- sqrt(diag(cov))

png(filename = "boyle_3_qq.png", width = 1000, height = 600)
qqnorm(rstandard(boyle_reg_3), main = "QQPlot of regression residuals, Judge Boyle model 3")
dev.off()

stargazer(boyle_reg_1, boyle_reg_2, boyle_reg_3, dep.var.labels=c("Above median sentence","Class 4 prison sentence", "Severe sentence"), se = list(boyle_reg_1_robust.se, boyle_reg_2_robust.se, boyle_reg_2_robust.se), align=TRUE, type = "html", omit = "sentence_year.f", notes = "Also controlling for sentence year fixed effects", omit.stat = c("rsq", "f", "ser"), out = "boyle_reg.html")

walowski_reg_1 <- lm(above_median ~ walowski_dummy + sentence_date + sentence_year.f, data = model_1_data)
cov <- vcovHC(walowski_reg_1, type = "HC") 
walowski_reg_1_robust.se <- sqrt(diag(cov))
walowski_reg_2 <- lm(counter_F4_pj ~ walowski_dummy + sentence_date + sentence_year.f, data = model_2_data)
cov <- vcovHC(walowski_reg_2, type = "HC") 
walowski_reg_2_robust.se <- sqrt(diag(cov))
walowski_reg_3 <- lm(severe_sentence ~ walowski_dummy + sentence_date + sentence_year.f, data = model_3_data)
cov <- vcovHC(walowski_reg_3, type = "HC") 
walowski_reg_3_robust.se <- sqrt(diag(cov))

stargazer(walowski_reg_1, walowski_reg_2, walowski_reg_3, dep.var.labels=c("Above median sentence","Class 4 prison sentence", "Severe sentence"), se = list(walowski_reg_1_robust.se, walowski_reg_2_robust.se, walowski_reg_2_robust.se), align=TRUE, type = "html", omit = "sentence_year.f", notes = "Also controlling for sentence year fixed effects", omit.stat = c("rsq", "f", "ser"), out = "walowski_reg.html")

araujo_reg_1 <- lm(above_median ~ araujo_dummy + sentence_date + sentence_year.f, data = model_1_data)
cov <- vcovHC(araujo_reg_1, type = "HC") 
araujo_reg_1_robust.se <- sqrt(diag(cov))
araujo_reg_2 <- lm(counter_F4_pj ~ araujo_dummy + sentence_date + sentence_year.f, data = model_2_data)
cov <- vcovHC(araujo_reg_2, type = "HC") 
araujo_reg_2_robust.se <- sqrt(diag(cov))
araujo_reg_3 <- lm(severe_sentence ~ araujo_dummy + sentence_date + sentence_year.f, data = model_3_data)
cov <- vcovHC(araujo_reg_3, type = "HC") 
araujo_reg_3_robust.se <- sqrt(diag(cov))

stargazer(araujo_reg_1, araujo_reg_2, araujo_reg_3, dep.var.labels=c("Above median sentence","Class 4 prison sentence", "Severe sentence"), se = list(araujo_reg_1_robust.se, araujo_reg_2_robust.se, araujo_reg_2_robust.se), align=TRUE, type = "html", omit = "sentence_year.f", notes = "Also controlling for sentence year fixed effects", omit.stat = c("rsq", "f", "ser"), out = "araujo_reg.html")

byrne_reg_1 <- lm(above_median ~ byrne_dummy + sentence_date + sentence_year.f, data = model_1_data)
cov <- vcovHC(byrne_reg_1, type = "HC") 
byrne_reg_1_robust.se <- sqrt(diag(cov))
byrne_reg_2 <- lm(counter_F4_pj ~ byrne_dummy + sentence_date + sentence_year.f, data = model_2_data)
cov <- vcovHC(byrne_reg_2, type = "HC") 
byrne_reg_2_robust.se <- sqrt(diag(cov))
byrne_reg_3 <- lm(severe_sentence ~ byrne_dummy + sentence_date + sentence_year.f, data = model_3_data)
cov <- vcovHC(byrne_reg_3, type = "HC") 
byrne_reg_3_robust.se <- sqrt(diag(cov))

stargazer(byrne_reg_1, byrne_reg_2, byrne_reg_3, dep.var.labels=c("Above median sentence","Class 4 prison sentence", "Severe sentence"), se = list(byrne_reg_1_robust.se, byrne_reg_2_robust.se, byrne_reg_2_robust.se), align=TRUE, type = "html", omit = "sentence_year.f", notes = "Also controlling for sentence year fixed effects", omit.stat = c("rsq", "f", "ser"), out = "byrne_reg.html")

raines_reg_1 <- lm(above_median ~ raines_dummy + sentence_date + sentence_year.f, data = model_1_data)
cov <- vcovHC(raines_reg_1, type = "HC") 
raines_reg_1_robust.se <- sqrt(diag(cov))
raines_reg_2 <- lm(counter_F4_pj ~ raines_dummy + sentence_date + sentence_year.f, data = model_2_data)
cov <- vcovHC(raines_reg_2, type = "HC") 
raines_reg_2_robust.se <- sqrt(diag(cov))
raines_reg_3 <- lm(severe_sentence ~ raines_dummy + sentence_date + sentence_year.f, data = model_3_data)
cov <- vcovHC(raines_reg_3, type = "HC") 
raines_reg_3_robust.se <- sqrt(diag(cov))

stargazer(raines_reg_1, raines_reg_2, raines_reg_3, dep.var.labels=c("Above median sentence","Class 4 prison sentence", "Severe sentence"), se = list(raines_reg_1_robust.se, raines_reg_2_robust.se, raines_reg_2_robust.se), align=TRUE, type = "html", omit = "sentence_year.f", notes = "Also controlling for sentence year fixed effects", omit.stat = c("rsq", "f", "ser"), out = "raines_reg.html")
```

Now, I bootstrap one of my models (I'm not doing all of them because that would take ages, and I see this as more of a sanity check to see if the coefficients/standard errors are wildly different). I'm choosing the third column of Judge Maura Slattery Boyle's regression table. The outputs are a summary of the coefficient, a plot of its distribution, and a confidence interval around it. 
...
summary(boyle_reg_3_boot)

png(file = "bootstrap_graphs.png", width = 1200, height = 700)
plot(boyle_reg_3_boot)
dev.off()
...
Happily, our bootstrap estimates fall in line well (almost perfectly in fact) with our coefficient from the regular OLS regression (see column 3 of our regression table for Judge Slattery Boyle for reference). I won't replicate this experiment with all the judges (I've put my laptop through enough) but I take this as a good sign that the sample size is mitigating the impacts of our wonky residuals, and therefore our coefficients for all models are pretty robust. 
