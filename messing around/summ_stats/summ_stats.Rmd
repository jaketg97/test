...
var1_query =
  glue_sql(
    "
    SELECT author_id, CONCAT(EXTRACT(WEEK FROM date(created_at)), EXTRACT(WEEK FROM date(created_at))) as week_year, COUNT(DISTINCT project_id) as num_projects FROM `ghtorrent-bq.ght.commits`
    group by author_id, CONCAT(EXTRACT(WEEK FROM date(created_at)), EXTRACT(WEEK FROM date(created_at)))
